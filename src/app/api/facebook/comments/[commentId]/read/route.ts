...
import { markCommentRead } from '@/lib/facebook/store';

// POST /api/facebook/comments/:commentId/read — for the separate marketing
// website. Call this when a comment is opened/viewed in the UI. Only
// transitions new → read (a no-op, not an error, if already read/replied) —
// fetching the comments list never marks anything read on its own.
...
export async function POST(request: Request, { params }: { params: Promise<{ commentId: string }> }) {
  ...
  if (!await rateLimit(ip, 'facebook-comments-read', 60, '1 m')) {
  ...
  try {
    ...
    console.error('[api/facebook/comments/[commentId]/read] error:', err instanceof Error ? err.message : err);
