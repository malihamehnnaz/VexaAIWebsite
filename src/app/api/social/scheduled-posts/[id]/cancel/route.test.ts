...
beforeEach(() => cancelScheduledPostMock.mockReset());

// The "cancelling a published post -> 409" case is covered in its own file
...
describe('POST /api/social/scheduled-posts/:id/cancel', () => {
  it('rejects unauthenticated', async () => {
    const response = await POST(new Request('https://www.vexaai.se/x', { method: 'POST' }), ctx('post-1'));
    expect(response.status).toBe(401);
  });

  it('is idempotent — cancelling an already-cancelled post returns success', async () => {
    cancelScheduledPostMock.mockResolvedValue({ id: 'post-1', status: 'cancelled' });
