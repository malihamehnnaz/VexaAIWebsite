...
// Split into its own file — see ../cancel/route.test.ts's comment for why.
...
describe('POST /api/social/scheduled-posts/:id/retry — rejects', () => {
  it('only allows retry for a failed post — rejects otherwise with 409', async () => {
    retryFailedPostMock.mockImplementation(async () => { throw new NotEditableError('scheduled'); });
