...
interface BlogPostContentProps {
  ...
  author: string | { en: string; sv: string };
  date: string;
  image?: string;
}
...
export default function BlogPostContent({ title, content, author, date, image }: BlogPostContentProps) {
  ...
  const resolvedContent = typeof content === 'string' ? content : content[language];
  const resolvedAuthor = typeof author === 'string' ? author : author[language];
    ...
    <article className="container mx-auto max-w-5xl px-4 py-14 md:py-16">
      ...
      <header className="mt-8 rounded-[2rem] border border-white/10 bg-gradient-to-br from-background to-muted/30 p-8 shadow-xl shadow-primary/5 md:p-10">
        <div className="mx-auto max-w-3xl">
          <h1 className="text-4xl font-bold tracking-tight leading-tight md:text-6xl">{resolvedTitle}</h1>
          <div className="mt-6 flex flex-wrap items-center gap-4 text-muted-foreground">
            <div className="flex items-center gap-3">
              <Avatar className="h-11 w-11 border border-border/60">
                <AvatarFallback>{resolvedAuthor.split(' ').map((part) => part[0]).join('').slice(0, 2)}</AvatarFallback>
              </Avatar>
              <div>
                <p className="font-medium text-foreground">{resolvedAuthor}</p>
                <p className="text-sm">{copy.editorial}</p>
              </div>
            </div>
      ...
      {image ? (
        <div className="mt-8 overflow-hidden rounded-[2rem] border border-white/10 shadow-2xl shadow-primary/5">
          <Image src={image} alt={resolvedTitle} width={1200} height={600} className="h-auto w-full object-cover" />
        </div>
      ) : null}
